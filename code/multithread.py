# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 12:20:51 2024
...
def parallel_glcm_computation(arr):
    num_threads = cpu_count() #change as required and available
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        img_slices = executor.map(process_image_band, get_indices())
        results = executor.map(calculate_glcm_properties, img_slices)
...
if __name__ == '__main__':
    ...
    for inpath in infiles:
        ...
        for band in range(img.shape[2]):
            ...
            # print('Time for one band is', bandend - bandstart)
            print('GLCM matrix size:', lev,'x',lev)
            print('Time taken with multithreading is ', bandend - bandstart)
            break
